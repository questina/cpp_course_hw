...
int add_toy(struct toy_array *store, char* toy_name, double toy_price, char* toy_country, int toy_amount){
    if (store == NULL){
        ...
        if (store->toys == NULL) {
            ...
            store->toys = realloc(store->toys, (store->size + 1) * sizeof(struct toy));
...
struct toy_array find_toys_spec_by_country(const struct toy_array store, const char* toy_country){
    struct toy_array suitable_toys = {0, NULL};
    for (int i = 0; i < store.size; i++){
        if ((store.toys[i].country == toy_country) && (store.toys[i].amount > 0)) {
            printf("name = %s, price = %f, amount = %d\n", store.toys[i].name, store.toys[i].price, store.toys[i].amount);
        }
    }
